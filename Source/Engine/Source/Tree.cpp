...
Tree::Tree(int min_size, int max_size, int map_size, int split_width) : min_size(min_size), max_size(max_size), split_width(split_width)
{
	min_size_before_split = min_size * 2 + split_width;
	to_check.push_back(GetNode(Int2::Zero, Int2(map_size)));
}
...
void Tree::Split(Node& node)
{
	...
	if(ratio >= 1.25f)
		node.horizontal_split = true;
	else if(ratio <= 0.75f)
		node.horizontal_split = false;
	else
		node.horizontal_split = Rand() % 2 == 0;

	int take0 = split_width / 2;
	int take1 = split_width - take0;
	if(node.horizontal_split)
	{
		...
		node.childs[0] = GetNode(node.pos, Int2(split - take0, node.size.y));
		node.childs[1] = GetNode(Int2(node.pos.x + split + take1, node.pos.y), Int2(node.size.x - split - take1, node.size.y));
	}
	else
	{
		...
		node.childs[0] = GetNode(node.pos, Int2(node.size.x, split - take0));
		node.childs[1] = GetNode(Int2(node.pos.x, node.pos.y + split + take1), Int2(node.size.x, node.size.y - split - take1));
	}
